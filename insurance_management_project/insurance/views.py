...
def home_view(request):
    if request.user.is_authenticated:
        return HttpResponseRedirect('afterlogin') #pending
...
def update_customer_view(request,pk):
    ...
    user=CMODEL.User.objects.get(id=customer.user_id)#what is this customer.user_id?
    ...
    if request.method=='POST':
        userForm=CFORM.CustomerUserForm(request.POST,instance=user) #explain this if user submitted form
...
def logout_view(request):
    if request.user.is_authenticated:
        logout(request)
        # Perform any additional actions you need to after logout
